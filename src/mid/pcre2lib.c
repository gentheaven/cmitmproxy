...
//regex replace
int regex_replace(char* content, unsigned int len, 
	char* regex_match, char* regex_rep, 
	char* chg_content, size_t* chg_len, int reg_extend_flag)
{
	...
	if (re == NULL) {
		...
		fprintf(stderr, "PCRE2 compile error: %s at offset %zu\n", buffer, error_offset);
		return -1;
	}

	PCRE2_SIZE subject_length = len;
	PCRE2_SIZE rep_leng = strlen(regex_rep);
	PCRE2_SIZE out_len = *chg_len;
	...
	if (result < 0) {
		...
		case PCRE2_ERROR_NOMEMORY:
			printf("regex_replace: not enought memory\n");
			break;
	...
	// string end with 0
	if (out_len < *chg_len) {
		chg_content[out_len] = '\0';
		*chg_len = out_len;
	} else {
		printf("regex_replace: need len=%lld, current len=%lld\n", out_len, *chg_len);
		return -1;
	}
